                ...
                >
                    <tbody>
                        <tr>
                            ...
                            >
                                <strong>GSTIN : </strong>
                                {purchaseOrder.companyGstin || settings.companyGstin}
                            </td>

                            {/* Right Cell — PURCHASE ORDER */}
                            <td
                                style={{
                                    border: '1px solid #000',
                                    borderLeft: 'none',
...
>
  <tbody>
    <tr>
      ...
      >
        E-Way Bill Date :{' '}
        {purchaseOrder.eWayBillDate ? formatDate(purchaseOrder.eWayBillDate) : ''}
      </td>
      <td
        style={{
          border: '1px solid #000',
  ...
  <tbody>
    <tr>
      ...
      >
        ...
        <div style={{ marginTop: '4mm' }}>
          <div style={{ fontWeight: '800' }}>
            <b>Contact No. :</b> {supplier.contact || ''}
          </div>
          <div style={{ marginTop: '1.5mm', fontWeight: '800' }}>
            <b>GSTIN :</b> {purchaseOrder.supplierGSTIN || settings.companyGstin}
          </div>
        </div>
      </td>
      ...
      >
        ...
        >
          Delta's Trade Link [{new Date(purchaseOrder.createdAt).getFullYear()}–
          {new Date(purchaseOrder.createdAt).getFullYear() + 1}]
        </div>

        <div style={{ marginBottom: '1.9mm', fontWeight: '400' }}>
          NO: 4078, THOTTANUTHU ROAD,<br />
          REDDIYAPATTI (POST), NATHAM ROAD,<br />
          DINDIGUL-624003, TAMIL NADU, INDIA.<br />
        ...
        >
          <tbody>
            <tr>
              ...
              <td style={{ fontWeight: '400' }}>: {purchaseOrder.poNumber}</td>
            </tr>
            <tr>
              <td style={{ fontWeight: '800' }}>Date</td>
              <td style={{ fontWeight: '400' }}>: {formatDate(purchaseOrder.createdAt)}</td>
            </tr>
            <tr>
              <td style={{ fontWeight: '800' }}>Disp.From</td>
              <td style={{ fontWeight: '400' }}>
                : {purchaseOrder.dispatchFrom || 'Main'}
              </td>
            </tr>
            <tr>
              <td style={{ fontWeight: '800' }}>No.of Pack</td>
              <td style={{ fontWeight: '400' }}>: {purchaseOrder.noOfPacks || ''}</td>
            </tr>
            <tr>
              <td style={{ fontWeight: '800' }}>Transport</td>
              <td style={{ fontWeight: '400' }}>: {purchaseOrder.transport || ''}</td>
            </tr>
          </tbody>
        </table>
      </td>
...
>
  <tbody>
    <tr>
      ...
      >
        <div>
          <b>Terms of Payment :</b> {purchaseOrder.paymentTerms || ''}
        </div>
        <div style={{ marginTop: '3mm' }}>
          <b>Destination :</b> {purchaseOrder.destination || ''}
        </div>
      </td>
      ...
      >
        <div>
          <b>P.O No & Date :</b> {purchaseOrder.poNumber} /{' '}
          {formatDate(purchaseOrder.createdAt)}
        </div>
        <div style={{ marginTop: '3mm' }}>
          <b>D.C No & Date :</b> {purchaseOrder.dcNo || ''}
        </div>
      </td>
      ...
      >
        <div>
          <b>Salesman :</b> {purchaseOrder.salesman || '<None>'}
        </div>
      </td>
    </tr>
  </tbody>
  ...
  {purchaseOrder.items.map((item, index) => {
    const hasData =
      item.itemCode || item.material?.name || item.hsn || item.quantity || item.rate;
    ...
    return (
      <tr key={item._id || index}>
        {[
          index + 1,
          item.itemCode || item.material?.code || '',
          item.material?.name || '',
          item.hsn || '48191010',
          `${item.gstPercent || 5}%`,
          'Nos',
          formatCurrency(item.quantity || 0),
          item.rate ? formatCurrency(item.rate) : '3.80',
  ...
  <tbody>
    {hsnList.map((hsn, index) => (
      <tr key={index}>
       ...
       {/* Right-side stacked Input CGST/SGST section */}
{/* Right-side stacked Input CGST/SGST section */}
{index === 0 && (
  <>
    <td
      rowSpan={hsnList.length}
      style={{
        borderTop: 'none',
        borderBottom: 'none',
        borderLeft: 'none',
        borderRight: '1px solid #000', // keep only right-side border
        textAlign: 'left',
        verticalAlign: 'top',
        padding: '1.5mm 2mm',
        fontSize: '8.5pt',
    ...
    >
      Input-CGST-{hsn.cgstRate}%<br />
      Input-SGST-{hsn.sgstRate}%
    </td>
    <td
      rowSpan={hsnList.length}
      style={{
        ...
        borderBottom: 'none',
        borderLeft: 'none',
        borderRight: '1px solid #000', // only right-side border visible
        textAlign: 'right',
        verticalAlign: 'top',
        padding: '1.5mm 2mm',
        fontSize: '8.5pt',
