                    ...
                    {purchaseOrders.length === 0 ? (
                        ...
                        purchaseOrders.map(po => (
                            <tr key={po._id} className="hover:bg-light-200">
                                ...
                                <td className="px-6 py-4 text-sm">
                                    <Link to={`/stock/maintenance/purchase-orders/${po._id}`} className="text-blue-500 hover:text-blue-700">
                                        <FaEye />
                                    </Link>
                                </td>
                            </tr>
...
const ViewStockPurchaseOrders = () => {
    ...
    useEffect(() => {
        const fetchPurchaseOrders = async () => {
            try {
                const { data } = await api.get('/purchase-orders?materialType=raw');
    ...
    return (
        <Card>
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-dark-700">Stock Maintenance Purchase Orders</h2>
                <div className="flex space-x-3">
                    <ViewReportTools
                        data={filteredPurchaseOrders}
                        title="Stock Maintenance Purchase Orders"
                        fileName="StockPOs"
                        metaDetails={{ user: 'Current User' }}
                        columns={reportColumns}
                    />
                    <Link to="/stock/maintenance/purchase-orders/create" className="px-4 py-2 text-white bg-primary-500 rounded-lg hover:bg-primary-600">
                        Create New PO
                    </Link>
                </div>
            </div>
...
export default ViewStockPurchaseOrders;