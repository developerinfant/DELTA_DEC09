...
import { FaSpinner, FaEdit, FaTrash, FaInfoCircle, FaCheck } from 'react-icons/fa';
    ...
    useEffect(() => {
        if (initialData && initialData._id) {
            setFormData({
                ...
                supplierType: initialData.supplierType || 'Packing Material Purchase Order',
                ...
                materialType: initialData.materialType || 'packing'
            });
        } else {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            className={commonInputClass}
                        >
                            <option value="Packing Material Purchase Order">Packing Material Purchase Order</option>
                            <option value="Raw Material Purchase Order">Raw Material Purchase Order</option>
                            <option value="Both (Packing + Raw) PO">Both (Packing + Raw) PO</option>
                            <option value="Jobber Packing Material">Jobber Packing Material</option>
                            <option value="Jobber Raw Material">Jobber Raw Material</option>
                            <option value="Both Jobber (Packing + Raw)">Both Jobber (Packing + Raw)</option>
...
const ManagePackingSuppliers = () => {
    ...
    const [newlyAddedSupplierId, setNewlyAddedSupplierId] = useState(null);
    
    // Ref to track if we're adding a new supplier
    const isAddingSupplier = useRef(false);
    ...
    const handleCreate = async (supplierData) => {
        isAddingSupplier.current = true;
        try {
            const { data } = await api.post('/suppliers', supplierData);
            setSuppliers([...suppliers, data]);
            setNewlyAddedSupplierId(data._id);
            showConfirmationMessage('Supplier created successfully and added to Packing Materials Supplier list.');
            
            // Reset form
            setTimeout(() => {
                setNewlyAddedSupplierId(null);
            }, 1000);
        } catch (error) {
            throw error;
        } finally {
            isAddingSupplier.current = false;
        }
    };
    ...
    // Filter suppliers for display - show packing material suppliers
    const packingSuppliers = suppliers.filter(supplier => 
        supplier.supplierType === 'Packing Material Purchase Order' ||
        supplier.supplierType === 'Both (Packing + Raw) PO' ||
        supplier.supplierType === 'Jobber Packing Material' ||
        supplier.supplierType === 'Both Jobber (Packing + Raw)' ||
        supplier.supplierType === 'All (Global)' ||
        (!supplier.supplierType && (!supplier.materialType || supplier.materialType === 'packing'))
    );
    ...
    const reportColumns = [
        ...
        { header: 'TYPE', key: 'supplierType', render: (value) => (
            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                value === 'Packing Material Purchase Order' ? 'bg-green-100 text-green-800' : 
                value === 'Raw Material Purchase Order' ? 'bg-blue-100 text-blue-800' : 
                value === 'Both (Packing + Raw) PO' ? 'bg-purple-100 text-purple-800' :
                value === 'Jobber Packing Material' ? 'bg-yellow-100 text-yellow-800' :
                value === 'Jobber Raw Material' ? 'bg-indigo-100 text-indigo-800' :
                value === 'Both Jobber (Packing + Raw)' ? 'bg-pink-100 text-pink-800' :
                'bg-gray-100 text-gray-800'
            }`}>
                {value}
            </span>
        ) },
        { header: 'Material', key: 'materialType', render: (value) => (
            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                !value ? 'bg-yellow-100 text-yellow-800' : 
                value === 'packing' ? 'bg-blue-100 text-blue-800' : 
                value === 'raw' ? 'bg-green-100 text-green-800' : 
                value === 'both' ? 'bg-purple-100 text-purple-800' :
                'bg-gray-100 text-gray-800'
            }`}>
                {!value ? 'Not Specified' : 
    ...
    return (
        <div className="space-y-6">
                ...
                <div>
                    {successMessage && (
                        <div className={`p-4 rounded-md flex items-center ${
                            ...
                            {showConfirmation && <FaCheck className="mr-2" />}
                            {successMessage}
                        </div>
                    )}
                </div>
                <ViewReportTools
                    data={packingSuppliers}
                    title="Packing Materials Suppliers"
                    fileName="PackingSuppliers"
            ...
            <Card title="Packing Materials Suppliers">
                <div className="mb-4 p-4 bg-blue-50 rounded-lg">
                    <p className="text-blue-800">
                        <strong>Note:</strong> This shared supplier list shows all suppliers. 
                        Suppliers marked with "Packing" or no type designation are suitable for packing materials.
                    </p>
                </div>
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-light-200">
                        ...
                        <tbody className="bg-light-100 divide-y divide-light-200">
                            {packingSuppliers.map(supplier => (
                                <tr 
                                    key={supplier._id} 
                                    className={`hover:bg-light-200 ${
                                        newlyAddedSupplierId === supplier._id 
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-dark-700 text-center">
                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                            supplier.supplierType === 'Packing Material Purchase Order' ? 'bg-green-100 text-green-800' : 
                                            supplier.supplierType === 'Raw Material Purchase Order' ? 'bg-blue-100 text-blue-800' : 
                                            supplier.supplierType === 'Both (Packing + Raw) PO' ? 'bg-purple-100 text-purple-800' :
                                            supplier.supplierType === 'Jobber Packing Material' ? 'bg-yellow-100 text-yellow-800' :
                                            supplier.supplierType === 'Jobber Raw Material' ? 'bg-indigo-100 text-indigo-800' :
                                            supplier.supplierType === 'Both Jobber (Packing + Raw)' ? 'bg-pink-100 text-pink-800' :
                                            'bg-gray-100 text-gray-800'
                                        }`}>
...
export default ManagePackingSuppliers;