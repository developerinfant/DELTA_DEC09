...
const createGRN = async (req, res) => {
    let { purchaseOrderId, deliveryChallanId, items, receivedBy, dateReceived, cartonsReturned, damagedStock, isTemporary } = req.body;
    ...
    try {
        ...
        if (deliveryChallanId) {
            ...
            if (cartonsReturned !== undefined) {
                ...
                // Damaged stock is now handled separately through the dedicated endpoint
                // This section has been removed to ensure damaged stock doesn't overwrite existing entries
...
const updateGRN = async (req, res) => {
    ...
    try {
        ...
        if (grn.sourceType === 'jobber' && grn.deliveryChallan && cartonsReturned !== undefined) {
            ...
            if (isNaN(cartonsReturned) || cartonsReturned <= 0) {
/**
 * @desc    Save damaged stock entries separately from GRN creation/update
 * @route   POST /api/grn/:id/damaged-stock
 * @access  Private
 */
const saveDamagedStock = async (req, res) => {
    try {
        ...
        // Find the GRN
        const grn = await GRN.findById(grnId).populate('deliveryChallan');
        if (!grn) {
            return res.status(404).json({ message: 'GRN not found.' });
        }
        
        // Validate damaged stock data
        if (!damagedStock || !Array.isArray(damagedStock) || damagedStock.length === 0) {
            return res.status(400).json({ message: 'Damaged stock data is required.' });
        }
        
        // Validate each damaged stock entry
        for (const item of damagedStock) {
            if (!item.material_name || item.damaged_qty === undefined || item.received_qty === undefined) {
                return res.status(400).json({ message: 'Each damaged stock entry must include material_name, received_qty, and damaged_qty.' });
            }
        ...
        // Get delivery challan info
        const dc = grn.deliveryChallan;
        if (!dc) {
            return res.status(400).json({ message: 'Delivery Challan not found for this GRN.' });
        }
        
        // Create damaged stock records
        const DamagedStock = require('../models/DamagedStock');
        const damagedStockPromises = damagedStock.map(item => {
            return new DamagedStock({
                ...
                remarks: 'Recorded separately from GRN'
            }).save();
        });
        
        const savedDamagedStockEntries = await Promise.all(damagedStockPromises);
        
        res.status(201).json({
            message: 'Damaged stock entries saved successfully.',
            data: savedDamagedStockEntries
        });
    } catch (error) {
        console.error(`Error saving damaged stock entries: ${error.message}`);
        res.status(500).json({ message: 'Server error while saving damaged stock entries.' });
    }
};
...
const getGRNItemsForDamagedStock = async (req, res) => {
    try {
        ...
        const damagedItems = grn.items.map(item => ({
            ...
            damaged_qty: item.damagedQuantity || 0, // Include damaged quantity from GRN
...
/**
 * @desc    Get damaged stock entries for a delivery challan
 * @route   GET /api/grn/damaged-stock/:dcId
 * @access  Private
 */
const getDamagedStockForDC = async (req, res) => {
    try {
        ...
        // Find all GRNs for this delivery challan
        const grns = await GRN.find({ deliveryChallan: dcId });
        const grnIds = grns.map(grn => grn._id);
        
        // Find all damaged stock entries for these GRNs
        const DamagedStock = require('../models/DamagedStock');
        const damagedStockEntries = await DamagedStock.find({ grn_id: { $in: grnIds } });
        
        res.json(damagedStockEntries);
    } catch (error) {
