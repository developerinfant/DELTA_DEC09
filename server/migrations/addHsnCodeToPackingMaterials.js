...
const addHsnCodeField = async () => {
  try {
    // Add hsnCode field to all existing packing materials that don't have it
    const result = await PackingMaterial.updateMany(
      { hsnCode: { $exists: false } },
      { $set: { hsnCode: '' } }
    );
    
    console.log(`Migration completed. Updated ${result.modifiedCount} packing materials with hsnCode field.`);
  } catch (err) {
